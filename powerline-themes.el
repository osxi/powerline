...
(defun powerline-bad-ass-theme ()
  "Setup the bad-ass mode-line.  Damn, I'm looking good."
  (interactive)
  (setq-default mode-line-format
                '("%e"
                  (:eval
                   (let* ((active (powerline-selected-window-active))
                          (mode-line (if active 'mode-line 'mode-line-inactive))
                          (face1 (if active 'powerline-active-dark 'powerline-inactive-dark))
                          (face2 (if active 'powerline-active-medium 'powerline-inactive-medium))
                          (separator-left (intern (format "powerline-%s-%s"
                                                          powerline-default-separator
                                                          (car powerline-default-separator-dir))))
                          (separator-right (intern (format "powerline-%s-%s"
